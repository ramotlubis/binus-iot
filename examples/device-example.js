...
function processTest(args) {
   ...
   timeout = setInterval(function() {
      count = ((Math.random() * 15) + 20).toFixed(2);

      if (args.testMode === 1) {
         const payloadName = "[SensorClientID:"+args.clientId+". SensorTime: "+Date.now()+"] Sensor Temperature ";
         device.publish('topic_2', JSON.stringify({
            Message: payloadName + ' = ' + count,
            Sender: args.clientId
         }));
      } else {
   ...
   device
      .on('message', function(topic, payload) {
         console.log("\n["+args.clientId+"-"+args.nim+"]New message received!");
         console.log("["+args.clientId+"-"+args.nim+"]Topic :"+topic);
         console.log("["+args.clientId+"-"+args.nim+"]Message Content:"+payload.toString());
      });
